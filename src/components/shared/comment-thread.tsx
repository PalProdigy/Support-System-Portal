...
import type { ThreadComment, Role } from '@/types'

// Cap how deep replies keep indenting so very deep threads don't run off-screen.
const MAX_INDENT_DEPTH = 4

// ── Pure helpers (exported so pages can build optimistic cache updates) ──────────
...
export function applyCommentReaction(
  comments: ThreadComment[], commentId: string, uid: string, reaction: 'like' | 'dislike',
): ThreadComment[] {
...
export function removeCommentAndDescendants(comments: ThreadComment[], commentId: string): ThreadComment[] {
...
// A reply is just a comment with a parent, so it renders itself for each of its
// children, giving every level the same actions (like / dislike / reply).
...
export function CommentThread({
  comments, currentUserId, onAddComment, onReact, onDelete, busyAdd, busyReact, busyDelete,
}: CommentThreadProps) {
  const [text, setText] = useState('')
  const topLevel = comments.filter((c) => (c.parent_id ?? null) === null)

  function submit() {
    const body = text.trim()
    if (!body) return
    onAddComment(body, null)
    setText('')
  }
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-end gap-2 border-t pt-3">
        <Textarea
          ...
          className="flex-1"
        />
        <Button disabled={!!busyAdd || !text.trim()} onClick={submit}>
          <Send className="h-3.5 w-3.5" /> {busyAdd ? 'Posting...' : 'Post'}
        </Button>
      </div>
    </div>
  )

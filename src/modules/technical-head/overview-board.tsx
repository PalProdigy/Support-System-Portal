...
import type { Case, Team, Client } from '@/types'
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-muted/30">
              {['Ref', 'Title', 'Priority', 'Status', 'Team', 'Client', 'SLA', ''].map((h) => (
            ...
            {filtered.map((c) => {
              ...
              return (
                <tr key={c.id} className="border-b last:border-0 hover:bg-accent/20 transition-colors">
                  ...
                  <td className="px-3 py-2.5 text-xs text-muted-foreground whitespace-nowrap">{teamsMap[c.team_id] ?? c.team_id}</td>

...
import { canWriteKB, KB_CATEGORIES } from '@/modules/kb/constants'
import { estimateReadingTimeMinutes, slugify } from '@/lib/markdown/utils'
import { cn, formatDate } from '@/lib/utils'
import {
  BookOpen, Clock, FolderOpen, MessageCircle, PenLine, PlusCircle,
  Tag, User as UserIcon, X,
} from 'lucide-react'
import type { KBArticle } from '@/types'

type Shelf = 'browse' | 'mine'
...
function KnowledgeBaseIndex() {
  ...
  const shelfArticles = shelf === 'mine' ? mine : publishedAll
  ...
  if (error) return <div><ErrorState onRetry={refetch} /></div>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-5">
      ...
      {writer && (
        <Tabs value={shelf} onValueChange={(v) => setShelf(v as Shelf)}>
          <TabsList className="w-full">
            <TabsTrigger value="browse" className="flex-1 gap-1.5"><BookOpen className="h-3.5 w-3.5" /> All Article ({publishedAll.length})</TabsTrigger>
            <TabsTrigger value="mine" className="flex-1 gap-1.5"><PenLine className="h-3.5 w-3.5" /> My Articles ({mine.length})</TabsTrigger>
          </TabsList>
        </Tabs>
      )}
      ...
      {isLoading ? (
        ...
        <EmptyState
          icon={BookOpen}
          title={query ? `No results found for "${query}"` : 'No articles found'}
          description={
            query || category || tag
...
function ArticleCard({ article: a, showStatus, onTag }: {
  ...
  return (
    <div className="group relative rounded-xl border bg-card p-4 flex flex-col gap-2 hover:shadow-md hover:border-primary/40 hover:-translate-y-0.5 transition-all">

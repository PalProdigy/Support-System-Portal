...
export function CreateCaseDialog({ open, onOpenChange, solution }: Props) {
  ...
  // When no solution is passed in (generic "Create Case"), let the client pick one of theirs.
  const needsSolutionPicker = !solution
  const { data: allSolutions } = useQuery({
    queryKey: ['solutions'],
    queryFn: () => dp.listSolutions(),
    enabled: open && needsSolutionPicker,
  })
  const { data: clientSolutions } = useQuery({
    queryKey: ['client-solutions', clientId],
    queryFn: () => dp.listClientSolutions(clientId),
    enabled: open && needsSolutionPicker && !!clientId,
  })
  const availableSolutions: Solution[] = (clientSolutions ?? []).flatMap((cs) => {
    const s = (allSolutions ?? []).find((x) => x.id === cs.solution_id)
    return s ? [s] : []
  })
  const effectiveSolution = solution ?? availableSolutions.find((s) => s.id === selectedSolutionId) ?? null

  const createMutation = useMutation({
    mutationFn: async () => {
      if (!clientId || !effectiveSolution) throw new Error('Missing client or solution')
      ...
      const newCase = await dp.createCase(
        {
          ...
          status: 'new',
          client_id: clientId,
          solution_id: effectiveSolution.id,
          team_id: teamId,
  ...
  const valid = form.title.trim().length >= 3 && form.description.trim().length >= 10 && !!effectiveSolution
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            Create Case
            {effectiveSolution && <span className="text-muted-foreground font-normal text-sm ml-1">· {effectiveSolution.name}</span>}
          </DialogTitle>
        </DialogHeader>
          ...
          {needsSolutionPicker && (
            <div className="space-y-1.5">
              ...
              <Select value={selectedSolutionId} onValueChange={setSelectedSolutionId}>
                <SelectTrigger>
                  <SelectValue placeholder={availableSolutions.length ? 'Select a solution…' : 'No solutions available'} />
                </SelectTrigger>
                <SelectContent>

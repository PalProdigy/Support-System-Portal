...
export default function TeamDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Mutation: submit a request (module_lead)
  ...
  // Mutation: submit a case transfer request (module_lead)
  ...
  const canManage = ['module_lead', 'technical_head'].includes(session.role)
            ...
            <div className="space-y-2">
              {members.map((m) => {
                ...
                return (
                  <div
                    key={m.id}
                    className="rounded-xl border bg-card p-3 flex items-start gap-3 cursor-pointer hover:bg-muted/30 transition-colors"
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        ...
        {!isTH && (
          <p className="text-xs text-muted-foreground bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
            As Module Lead, you can send a transfer request for approval by the Technical Head. The destination team will be decided by the Technical Head.
          </p>
        )}
